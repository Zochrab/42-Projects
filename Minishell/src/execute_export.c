...
void	print_export(t_data *data, t_env *tmp, t_cmd_list *cmd)
{
	...
	while (to_print(tmp))
	{
		smallest = get_smal(tmp);
		write(cmd->fd_out, "declare -x ", 11);
		write(cmd->fd_out, smallest->name, ft_strlen(smallest->name));
		if (smallest->equal[0] != '\0')
		{
			write(cmd->fd_out, smallest->equal, ft_strlen(smallest->equal));
			write(cmd->fd_out, "\"", 1);
			write(cmd->fd_out, smallest->value, ft_strlen(smallest->value));
			write(cmd->fd_out, "\"", 1);
		}
		write(cmd->fd_out, "\n", 1);
		tmp = tmp->next;
	}
	reset_print(data->first_env);
	return ;
}
