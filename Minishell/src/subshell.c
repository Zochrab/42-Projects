...
int	create_subshell(t_data *data)
{
	...
	if (!tmp)
		return (0);
	if (data->command_count <= 1 && tmp->name_token
		&& parent(data, tmp->name_token->str) == 1)
		return (delegate(data, tmp));
...
int	delegate(t_data *data, t_cmd_list *cmd)
{
	...
	else if (ft_strncmp(cmd->name, "./minishell", 11) == 0)
		data->status = exit_on_error(data, NMS, 1, 127);
	else if (!cmd->name || !ft_strncmp(cmd->name, "", ft_strlen(cmd->name)))
		data->status = exit_on_error(data, CNF, 1, 127);
	else
		exec(data, cmd);
	return (0);
}
...
int	subshell(t_data *data, int index)
{
	...
	while (cmd->index != index)
		cmd = cmd->next;
	if (cmd->eof == 1)
		exit_on_error(NULL, "Heredoc: End of line", 0, 0);
	else if (cmd->error != NULL)
		exit_on_error(data, cmd->error, data->parent + 1, 1);
	if (cmd->name_token && cmd->error == NULL)
		delegate(data, cmd);
	if (cmd->pid == 0)
	{
		cleanup(data);
