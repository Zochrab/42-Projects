...
int	numeric(t_cmd_list *cmd)
{
	...
	while (arg)
	{
		if ((arg->str == NULL || arg->str[0] == '\0') && arg->env)
		{
			arg = arg->next;
			continue ;
		}
		else if (arg->str == NULL || arg->str[0] == '\0')
			return (2);
		else if (arg->str[0] == '-' && arg->str[1] == '-' && !arg->str[2])
			return (0);
		else if ((arg->str[0] == '+' || arg->str[0] == '-')
			&& only_digits(arg->str + 1))
			return (0);
		else if (!ft_isdigit(arg->str[0]))
			return (2);
		arg = arg->next;
	}
	return (0);
...
int	handle_exit(t_data *data, t_cmd_list *cmd)
{
	...
	nbr = data->status;
	if (data->parent == 1 && isatty(STDIN_FILENO))
		write(1, "exit\n", 5);
